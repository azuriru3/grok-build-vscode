...
3. `session/update` twice with `update.sessionUpdate: "available_commands_update"`. Not one of the 5 types documented on agentclientprotocol.com/protocol/v1/prompt-turn.md. Carries the same `availableCommands` array seen in `initialize`'s `_meta`, plus a `loop` command not present at init time.
...
- Don't build a closed enum of `session/update.update.sessionUpdate` values. The real stream mixes documented types (`plan`, `agent_message_chunk`, `tool_call`, `tool_call_update`, `usage_update`, still unverified live) with undocumented ones (`available_commands_update`, `user_message_chunk`) that xAI actually sends. The client renders unknown `sessionUpdate` types as a generic raw JSON log entry instead of dropping or crashing on them.
